...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:variable name="metadata"
                select="/root/Indicators"/>


  <!-- Specific schema rendering -->
  <xsl:template mode="getMetadataTitle" match="Indicators">
    <xsl:value-of select="title"/>
  </xsl:template>

  <xsl:template mode="getMetadataHierarchyLevel" match="Indicators">
    <xsl:value-of select="'dataset'"/>
  </xsl:template>

  <xsl:template mode="getMetadataHeader" match="Indicators">
  </xsl:template>
  ...
  <!-- Traverse the tree -->
  <xsl:template mode="render-field" match="Indicators">
